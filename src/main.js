...
document.addEventListener("click", (e) => {
	if (playing) {
		...
		bullets.push(
			new Bullet(canvas.width / 2, canvas.height / 2, 5, "red", { x: velocity.x, y: velocity.y, })
		);
	}
});
//start button
...
const scenePlaying = () => {
	...
	bullets.forEach((bullet, bulleti) => {
		...
		if (
			bullet.x < 0 ||
			bullet.x + bullet.r > canvas.width ||
			bullet.y < 0 ||
			bullet.y + bullet.r > canvas.height
		) {
		...
		bullets.forEach((bullet, bulleti) => {
			...
			if (dist - enemy.r - bullet.r <= 0) {
				if (enemy.r - 10 > 10 && !powered.kill.on) {
					...
					window.setTimeout(() => { bullets.splice(bulleti, 1); });
				} else {
					if (difficulty < 30) {
					...
					window.setTimeout(() => {
						spawnParticles(bullet.x, bullet.y, `hsl(${enemy.color},100%,50%)`, 25, particles);
						bullets.splice(bulleti, 1);
